...
int server_setup_2() {
  ...
  hints->ai_socktype = SOCK_STREAM;  //TCP socket
  hints->ai_flags = AI_PASSIVE;  //Use all valid addresses
  getaddrinfo(NULL, PORT_2, hints, &results); //NULL means use local address
...
int server_connect_init(int sd,int send) {
  ...
  error_check(client_socket, "server accept");
    ...
    if (send) {//second handshake
write(client_socket,"succesfully established connection, connecting to first client",BUFFER_SIZE);

  }
  else {
    write(client_socket,"succesfully established connection, waiting for second client...",BUFFER_SIZE);
  }
...
int client_setup(char * server) {
  ...
  hints->ai_family = AF_INET;  //IPv4
  hints->ai_socktype = SOCK_STREAM;  //TCP socket
  getaddrinfo(server, PORT_2, hints, &results);
